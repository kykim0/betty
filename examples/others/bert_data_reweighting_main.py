...
class BERTEngine(Engine):
    ...
    def validation(self):
        ...
        if not args.retrain and not args.baseline:
            basedir = "/home/kykim/dev/checkpoints/betty/bert_data_reweighting"
            torch.save(self.finetune.state_dict(), f"{basedir}/net_{self.global_step}.pt")
            # torch.save(self.finetune.state_dict(), f"save/net_{self.global_step}.pt")
            torch.save(self.reweight.state_dict(), f"{basedir}/meta_net_{self.global_step}.pt")
            # torch.save(self.reweight.state_dict(), f"save/meta_net_{self.global_step}.pt")
